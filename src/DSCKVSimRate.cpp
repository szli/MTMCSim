...
extern const string default_pdfdir;

DSCKVSimRate::DSCKVSimRate(DSCKVRatePara& para_,TextSaveUI& resFile_, TextSaveUI& tempFile_, DispUI& dispUI_)
	: nbSize(para_.nbSize), codeLen(para_.codeLen), infoLen(para_.infoLen),
	maxErrFr(para_.maxFr), lambda(para_.lambda),
...
double DSCKVSimRate::KVLooseBound(const double &cost) // Corollary 5 in KV paper
{
	return(sqrt((double) 2 * (infoLen - 1) * cost));
}

double DSCKVSimRate::KVLooseBound(const double &cost, const int infoLen_)
{
	return(sqrt((double) 2 * (infoLen_ - 1) * cost));
}

double DSCKVSimRate::KVTightBound(const double &cost, const int infoLen_)
{
	double delta;
	double term1, term2;
	
	double looseB = KVLooseBound(cost, infoLen_);

	for(delta=0; delta<=looseB; delta++)
	{
		
		term1 = ceil((delta + 1) / (infoLen_ - 1));
		
		term2 = (int) floor((double) delta / (infoLen_ - 1));

		if ((term1 * (delta + 1 - (infoLen_ - 1) * term2 / 2)) > cost)
		{
			return(delta);
		}
	}
...
double DSCKVSimRate::KVTightBound(const double &cost) // Lemma 1 in KV paper
{
	double delta;
	double term1, term2;
...
void DSCKVSimRate::oneStep(const std::vector<int> &xSeq, const std::vector<int> &ySeq, ThreadWorkSpace& ws)
{
...
//#define HermitianGSDSC
	...
	int tryInfo = infoLen;
	double RHS;
	for (tryInfo = infoLen; tryInfo>=0; tryInfo--)
	{
		RHS = KVTightBound(cost, tryInfo);
		if (score > RHS)
			break;
	}
...
void DSCKVSimRate::oneThread(shared_ptr<RNObj> pobj, const MCThreadInfo& threadInfo)
{
...
void DSCKVSimRate::dumpOneResult(const MCThreadInfo& threadInfo)
{
	ThreadWorkSpace& ws = *(workspaceVec[threadInfo.threadID]);
	...
	double actTxRate = (codeLen- ws.infoLen)/ (double)codeLen * static_cast<int>(log((double)nbSize) / log(2.0) + 0.5);

	simuRes.totTxRate += actTxRate;


	simuRes.totFr++;
	simuRes.aveTxRate = simuRes.totTxRate / simuRes.totFr;

	//simuRes.aveInfoLen = (double)simuRes.totInfoLen / (double) simuRes.totFr;
	//simuRes.aveTxRate = (codeLen-simuRes.aveInfoLen)/codeLen * static_cast<int>(log((double)nbSize) / log(2.0) + 0.5);
	...
	double sum = 0;
		...
		{
			ostringstream ress;
			ress << "Frame: " << simuRes.totFr <<  
				"Ave Tx Rate " << simuRes.aveTxRate << " Stdev" << simuRes.stdev << "\n";
		...
		if (simuRes.totFr % para.dispPerFr == 0)
		{
			ostringstream ress;
			ress << "Frame: " << simuRes.totFr << 
				"Ave Tx Rate " << simuRes.aveTxRate <<  " Stdev" << simuRes.stdev << "Curr InfoLen" << ws.infoLen << "\r";
			dispUI.dispStr(ress.str());
		}

}

void DSCKVSimRate::postProcess()
{
		ostringstream ress;
...
void DSCKVSimRate::prepare(const MCThreadInfo& threadInfo)
{
...
shared_ptr<RNObj> DSCKVSimRate::genRNObj()
{
...
void DSCKVSimRate::DSCKVRatePara::loadPara(const paraMap& para, DispUI& ui)
{
	...
	if (!getIntPara(para, "max error frame", maxFr))
...
void DSCKVSimRate::setCompResMap(compResMap& compResM_) const
{
	compRes.setMapping(compResM_);
}


void DSCKVSimRate::DSCKVCompRes::setMapping(compResMap &crMap) const
{
...
bool DSCKVSimRate::checkTerm() 
{
	static vector<int> usedInfoLen;
	if (mulMode == 0) // not multi-run mode
	{
		usedInfoLen.clear();
		return (para.maxFr == simuRes.totFr);
	}
	else
	{
		/*
		shared_ptr<DSCKVRatePara> npara(boost::static_pointer_cast<DSCKVRatePara, SimuPara>(nextPara));//nextPara is a copy of a DSCKVPara obj, so cast is safe
