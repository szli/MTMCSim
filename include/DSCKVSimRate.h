#ifndef _DSCKVRate_H_INCLUDED
#define _DSCKVRate_H_INCLUDED
...
namespace MTMCSim
{
class DSCKVSimRate: public MultiRunSimuBase
{
public:
	...
	*/
	...
	class DSCKVRatePara: public SimuPara{
	public:
	...
	//	string saveFN; ///< The file  name of the result file. 
		...
		int maxFr; ///< maximum error frame (termination condition).
		...
		*/
		...
		{ return shared_ptr<DSCKVRatePara>(new DSCKVRatePara(*this)); }

		DSCKVRatePara():rngTableOpt(0) ///< Default constructor is used to set default values
		{}
	};
	...
	public:
		...
		DSCKVSimuRes():aveTxRate(0),totFr(0),totTxRate(0)
		{}
	};

	DSCKVSimRate(DSCKVRatePara& para_, TextSaveUI& resFile_, TextSaveUI& tempFile_, DispUI& dispUI_);
...
private:
	...
	double KVLooseBound(const double &cost, const int infoLen_);

	double KVTightBound(const double &cost, const int infoLen_);
