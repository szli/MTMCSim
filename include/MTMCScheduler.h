...
private:
    ...
    boost::shared_ptr<MTMCSimBase> MTMCSimulator; // The scheduler object does not own simulator object, if we want it to own, need a wrapper.

    vector<boost::shared_ptr<boost::thread> >  MCRunThreads;

    queue<boost::shared_ptr<RNObj> > RNQueue;

    boost::shared_ptr<boost::thread> RNGThread;
    ...
    boost::condition_variable CVQueueFull;
    
    boost::condition_variable CVQueueEmpty;

    boost::shared_mutex termMutex;
